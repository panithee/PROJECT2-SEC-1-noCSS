    ...
    <div v-show="showmenu == false" className=" h-screen m-[50px]">
      ...
      <div v-for="(meal, index) in g_list.meals">
        <div class="border border-black rounded-lg m-[20px]">
          ...
          <button class="border border-black m-[20px] rounded-lg p-1 text-green-800 hover:bg-black hover:text-green-300" @click="sw('edit', meal, index)">แก้ไขรายการ</button>
      ...
      <div class=" flex justify-center m-[20px] ">
        <button @click="sw('add')" class="bg-white shadow-md rounded-lg p-2 hover:bg-cyan-700 hover:text-white">เพิ่มมื้ออาหาร</button>
      </div>
    </div>
